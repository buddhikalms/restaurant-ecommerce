  ...
  const tabs: Array<{ id: TabId; label: string; content: string | null }> = [
    {
      id: "information",
      label: "Information",
      content: normalizeContent(information),
    },
    {
      id: "ingredients",
      label: "Ingredients",
      content: normalizeContent(ingredients),
    },
    {
      id: "nutritional",
      label: "Nutritional",
      content: normalizeContent(nutritional),
    },
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-3 font-heading text-3xl font-semibold text-slate-900">
            Everything buyers need before ordering
          </h2>
        </div>
        <p className="max-w-2xl text-sm leading-6 text-slate-600">
          Browse the product information, ingredients, nutritional notes, and
          FAQs added by the store team.
        </p>
      </div>
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={cn(
              ...
              activeTab.id === tab.id
                ? "border-[var(--brand)] bg-[var(--brand-dark)] text-white shadow-[0_12px_32px_rgba(155,95,25,0.22)]"
      ...
      <div className="mt-6 rounded-[2rem] border border-[var(--brand)]/12 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(255,248,238,0.92))] p-6 lg:p-7">
        ...
        {paragraphs.length ? (
          ...
          <p className="mt-4 text-sm leading-7 text-slate-500">
            No {activeTab.label.toLowerCase()} has been added for this product
            yet.
          </p>
        )}
      </div>

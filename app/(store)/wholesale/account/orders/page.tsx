  ...
  return (
    <div className="page-shell py-12">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--brand)]">Wholesale Orders</p>
          <h1 className="mt-3 font-heading text-4xl font-semibold text-slate-900">Order history</h1>
          <p className="mt-3 text-sm leading-6 text-slate-600">Track current fulfillment status and review previous wholesale purchases.</p>
        </div>
                  ...
                  {orders.map((order) => (
                    <tr key={order.id}>
                      <td className="px-6 py-4 font-semibold text-slate-900">
                        <Link href={`/wholesale/account/orders/${order.id}`}>{order.orderNumber}</Link>
                      </td>
